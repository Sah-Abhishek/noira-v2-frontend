  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      if (response.status == 200) {
        const profile = response.data.therapist?.userId || response.data;

        localStorage.setItem("userEmail", data.email);
        localStorage.setItem(`${role}jwt`, response.data.token);
        localStorage.setItem(`${role}Id`, response.data.therapistId || response.data.AdminId);
        localStorage.setItem("userId", response.data.userId);
        localStorage.setItem("firstname", response.data.name?.first || "Admin");
        setUser(profile);
      ...
      if (error.response) {
        ...
        setErrorMsg(`No response from server at ${apiUrl}. Make sure the backend is running.`);
      } else {
