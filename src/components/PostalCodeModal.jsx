  ...
  useEffect(() => {
    ...
    const fetchSuggestions = async () => {
      ...
      try {
        ...
        // Pull unique outcodes (e.g. "SW10" from "SW10 0AA") so the user can
        // pick the area instead of a specific postcode if they prefer.
        const extractedOutcodes = [
          ...new Set(postcodes.map((pc) => pc.split(" ")[0])),
        ];

        // If what the user typed already looks like a valid outcode
        // (e.g. "SW1", "W1A", "EC1"), include it too — even if no full
        // postcodes were returned for that exact stem.
        const typed = postalCode.trim().toUpperCase();
            ...
            <form onSubmit={handleSubmit} className="space-y-5 relative">
              <div className="relative">
                ...
                {showSuggestions && suggestions.length > 0 && (
                  <ul className="absolute z-20 top-full left-0 right-0 bg-gray-900 border border-gray-700 rounded-md mt-1 max-h-56 overflow-y-auto shadow-lg">
                    {suggestions.map((s, idx) => (
                      <li
                        key={idx}
                        onClick={() => handleSelectSuggestion(s.value)}
                        className="px-4 py-2 cursor-pointer hover:bg-primary/20 flex items-center justify-between"
                      >
                        <span>{s.value}</span>
                        {s.type === "outcode" && (
                          <span className="text-xs text-primary/70 ml-3 uppercase tracking-wide">
                            Area
                          </span>
                        )}
                      </li>
